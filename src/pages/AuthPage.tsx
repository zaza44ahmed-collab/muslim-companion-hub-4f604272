  ...
  if (isForgotPassword) {
    return (
      <div className="min-h-screen islamic-bg-light flex flex-col items-center justify-center px-4 py-8" dir="rtl">
        ...
        <h2 className="text-xl font-bold text-foreground mb-4 font-cairo">استعادة كلمة المرور</h2>
        <p className="text-sm text-muted-foreground mb-6 text-center max-w-sm">أدخل بريدك الإلكتروني وسنرسل لك رابط إعادة تعيين كلمة المرور</p>
        <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
          <div className="relative">
            ...
            <Input type="email" placeholder="البريد الإلكتروني" value={email} onChange={(e) => setEmail(e.target.value)}
              className="pr-11 text-right h-12 rounded-xl border-2 border-primary/30 focus:border-primary bg-card text-base" required />
          </div>
        ...
        <button onClick={() => setIsForgotPassword(false)} className="mt-4 text-sm text-primary hover:underline font-cairo">
    ...
    <div className="min-h-screen islamic-bg-light flex flex-col items-center justify-center px-4 py-8" dir="rtl">
      ...
      <div className="w-full max-w-sm flex rounded-xl bg-muted p-1 mb-6">
        <button onClick={() => setIsLogin(true)}
          className={`flex-1 py-2.5 rounded-lg text-sm font-semibold font-cairo transition-all ${isLogin ? "bg-card text-foreground shadow-sm" : "text-muted-foreground"}`}>
          تسجيل الدخول
        </button>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
        {!isLogin && (
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input type="text" placeholder="الاسم الأول" value={firstName} onChange={(e) => setFirstName(e.target.value)}
                className="pr-11 text-right h-12 rounded-xl border-2 border-primary/30 focus:border-primary bg-card text-base" required />
            </div>
            <div className="relative flex-1">
              <Input type="text" placeholder="الاسم الأخير" value={lastName} onChange={(e) => setLastName(e.target.value)}
                className="text-right h-12 rounded-xl border-2 border-primary/30 focus:border-primary bg-card text-base" required />
            </div>
          </div>
        )}
        ...
        <div className="relative">
          ...
          <Input type={showPassword ? "text" : "password"} placeholder="كلمة المرور" value={password} onChange={(e) => setPassword(e.target.value)}
            className="pr-11 pl-10 text-right h-12 rounded-xl border-2 border-primary/30 focus:border-primary bg-card text-base" minLength={6} required />
        </div>
        ...
        {!isLogin && (
          <div className="relative">
            ...
            <Input type={showConfirmPassword ? "text" : "password"} placeholder="تأكيد كلمة المرور" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
              className="pr-11 pl-10 text-right h-12 rounded-xl border-2 border-primary/30 focus:border-primary bg-card text-base" minLength={6} required />
          </div>
        )}
        ...
        {isLogin && (
          <div className="text-right">
            <button type="button" onClick={() => setIsForgotPassword(true)} className="text-sm text-primary hover:underline font-cairo">نسيت كلمة المرور؟</button>
          </div>
        )}
      ...
      <div className="mt-5 text-center">
        <button onClick={() => setIsLogin(!isLogin)} className="text-sm font-cairo text-muted-foreground">
          {isLogin ? "ليس لديك حساب؟ " : "لديك حساب؟ "}
          <span className="text-primary font-bold hover:underline">{isLogin ? "إنشاء حساب" : "تسجيل الدخول"}</span>
        </button>
      </div>
