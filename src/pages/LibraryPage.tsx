...
const LibraryPage = () => {
  ...
  const openUserBook = (book: any) => {
    if (book.file_url) {
      window.open(book.file_url, "_blank");
    } else {
      toast({ title: "لا يوجد ملف للكتاب" });
    }
  };
      ...
      <main className="container py-3 px-4">
              ...
              {filteredUserBooks.map((book, index) => (
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    {book.pages > 0 && <span className="text-xs text-muted-foreground">{book.pages} صفحة</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {filteredBooks.map((book, index) => (
            ...
            >
              <div className="aspect-[3/4] bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center relative overflow-hidden">
                <img src={book.cover} alt={book.title} className="h-full w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "/placeholder.svg"; }} />
                <span className="absolute top-2 right-2 px-2 py-0.5 bg-secondary text-secondary-foreground text-xs font-bold rounded">{book.format}</span>
              ...
              <div className="p-3">
                ...
                <p className="text-xs text-muted-foreground mb-2">{book.author}</p>
                <div className="flex items-center justify-between mb-3">
                  <span className="flex items-center gap-1 text-xs text-gold"><Star className="h-3 w-3 fill-gold" />{book.rating}</span>
                  <span className="text-xs text-muted-foreground">{book.pages} صفحة</span>
                </div>
