...
import { ArrowLeft, Sun, Plane, Heart, GraduationCap, Utensils, Baby, Share2, Copy, Check, Shield, Sparkles, BookOpen } from "lucide-react";
...
const DuasPage = () => {
  ...
  const handleShare = async (dua: { text: string; source: string }) => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: "دعاء من حقيبة المسلم", text: shareText }); }
      catch (err) { if ((err as Error).name !== "AbortError") copyToClipboard(shareText); }
      ...
      <div className="min-h-screen bg-background" dir="rtl">
        ...
        <main className="container py-4 space-y-4 pb-8">
          {selectedCategory.duas.map((dua, index) => (
            <div key={dua.id} className="bg-card rounded-2xl p-5 shadow-card-islamic animate-fadeIn" style={{ animationDelay: `${index * 80}ms` }}>
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="container flex h-14 items-center justify-between">
          <h1 className="text-lg font-bold font-amiri">الأدعية</h1>
          <Link to="/"><Button variant="ghost" size="icon"><ArrowLeft className="h-5 w-5" /></Button></Link>
        </div>
      </header>
      <main className="container py-6">
        <p className="text-center text-muted-foreground mb-6 text-sm">اختر المناسبة للوصول إلى الأدعية المناسبة</p>
